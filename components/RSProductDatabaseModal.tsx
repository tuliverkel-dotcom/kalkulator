...
interface RSProductDatabaseModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const RSProductDatabaseModal: React.FC<RSProductDatabaseModalProps> = ({
  isOpen,
  onClose,
  onSelectProduct,
  products,
  ...
  const [activeCategory, setActiveCategory] = useState('Rozvádzač');
    ...
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="p-4 border-b flex justify-between items-center bg-gray-50 rounded-t-xl">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-gray-800">
                {configuringProduct ? 'Konfigurácia Produktu (RS)' : 'Katalóg Produktov RS'}
            </h2>
          </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50 flex flex-col">
                ...
                <div className="bg-white p-6 rounded-xl border border-purple-200 shadow-sm mb-6">
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <h3 className="text-xl font-bold text-purple-900 mb-1">{configuringProduct.name}</h3>
                ...
                <div className="mt-auto bg-white p-4 rounded-xl border border-gray-200 flex justify-between items-center sticky bottom-0 shadow-lg">
                    <div>
                        ...
                        <div className="text-2xl font-bold text-purple-600">{calculateCurrentPrice().toFixed(2)} €</div>
                    </div>
                    <div className="flex gap-2">
                        ...
                        <button 
                            ...
                            className="px-6 py-2 bg-purple-600 text-white font-bold rounded hover:bg-purple-700 shadow flex items-center gap-2"
                        >
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
          ...
          {isAdding ? (
            <div className="bg-white p-6 rounded-xl border border-purple-200 shadow-md animate-in fade-in slide-in-from-top-2">
              <h3 className="font-bold text-purple-800 mb-6 text-lg border-b pb-2">Nový Master Produkt: {activeCategory}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div className="md:col-span-2">
                    ...
                    <input 
                    className="w-full p-2 border rounded text-sm font-bold focus:ring-2 focus:ring-purple-500 outline-none" 
                          ...
                          <div key={idx} className="bg-white border rounded p-3 relative">
                              ...
                              <div className="flex flex-wrap gap-2">
                                  {attr.options.map((opt, oIdx) => (
                                      <span key={oIdx} className="text-xs bg-purple-50 text-purple-700 px-2 py-1 rounded border border-purple-100">
                                          {opt.label} {opt.priceDelta > 0 ? `(+${opt.priceDelta}€)` : ''}
                                      </span>
                                  ))}
                              </div>
              ...
              <div className="flex justify-end gap-2 border-t pt-4">
                ...
                <button onClick={handleSaveNew} className="px-6 py-2 text-sm bg-purple-600 text-white rounded hover:bg-purple-700 flex items-center gap-2 font-bold shadow-sm">
             ...
             <button 
              ...
              className="w-full py-4 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 hover:border-purple-400 hover:text-purple-600 hover:bg-purple-50 transition flex justify-center items-center gap-2 mb-4 font-bold text-lg"
            >
          ...
          <div className="space-y-2">
            {filteredProducts.length === 0 && !isAdding ? (
              ...
              !isAdding && filteredProducts.map(product => (
                <div key={product.id} className="bg-white p-4 rounded-lg border border-gray-200 hover:border-purple-400 hover:shadow-md transition-all flex justify-between items-center group">
                  <div>
                    <div className="font-bold text-gray-800 text-lg flex items-center gap-2">
                        {product.name}
                        {product.attributes && product.attributes.length > 0 && (
                            <span className="text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full border border-purple-200 flex items-center gap-1 font-bold">
                    ...
                    <div className="text-sm text-gray-500 mt-1">Základ: <span className="font-bold text-purple-600">{product.price.toFixed(2)} €</span> / {product.unit}</div>
                  </div>
                  <div className="flex gap-2">
                     <button 
                    ...
                    <button 
                      ...
                      className="px-4 py-2 bg-gray-100 hover:bg-purple-600 hover:text-white text-gray-700 text-sm font-bold rounded transition border border-gray-200 flex items-center gap-2"
                    >
                      {product.attributes && product.attributes.length > 0 ? (
